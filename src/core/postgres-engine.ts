...
export class PostgresEngine implements BrainEngine {
  ...
  async putPage(slug: string, page: PageInput, opts?: { sourceId?: string }): Promise<Page> {
    ...
    const rows = await sql`
      INSERT INTO pages (source_id, slug, type, page_kind, title, compiled_truth, timeline, frontmatter, content_hash, updated_at, effective_date, effective_date_source, import_filename, chinese_search_vector, chunker_version, source_path, source_kind, source_uri, ingested_via, ingested_at)
      VALUES (${sourceId}, ${slug}, ${page.type}, ${pageKind}, ${page.title}, ${page.compiled_truth}, ${page.timeline || ''}, ${sql.json(frontmatter as Parameters<typeof sql.json>[0])}, ${hash}, now(), ${effectiveDate}, ${effectiveDateSource}, ${importFilename}, to_tsvector('simple', ${chineseSearchVector}), COALESCE(${chunkerVersion}::smallint, ${MARKDOWN_CHUNKER_VERSION}), ${sourcePath}, ${sourceKind}, ${sourceUri}, ${ingestedVia}, ${ingestedAt})
  ...
  async searchKeyword(query: string, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    // FTS config name for the non-CJK path (e.g. 'english', 'pt_br').
    // Validated by getFtsLanguage() — safe to interpolate into raw SQL.
    ...
    if (queryHasChinese) {
      try {
        ...
        const rows = await this.withScopedReadTransaction(opts?.sourceIds, opts?.sourceId, async (tx) => {
          ...
          return await tx.unsafe(jiebQuery, params as Parameters<typeof tx.unsafe>[1]);
        ...
        const rows = await this.withScopedReadTransaction(opts?.sourceIds, opts?.sourceId, async (tx) => {
          await tx`SET LOCAL statement_timeout = '8s'`;
          return await tx.unsafe(ilikeQuery, ilikeParams as Parameters<typeof tx.unsafe>[1]);
        }, { alwaysTransaction: true });
        return rows.map(rowToSearchResult);
      }
    }
  ...
  async getHealth(): Promise<BrainHealth> {
    ...
    const [h] = await sql`
      WITH entity_pages AS (
        SELECT id, slug FROM pages WHERE type IN ('person', 'company', 'organization', 'entity')
      )
      SELECT
