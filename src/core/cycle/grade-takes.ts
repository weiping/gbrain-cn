...
class GradeTakesPhase extends BaseCyclePhase {
  ...
  protected async process(
    ...
    // Fork extension (gbrain-cn): GBRAIN_GRADE_TAKES_MODEL env, if set, pins
    // the judge model per-phase ahead of the gateway chat model.
    const judgeModelFull = opts.model ?? process.env.GBRAIN_GRADE_TAKES_MODEL ?? getChatModel();
